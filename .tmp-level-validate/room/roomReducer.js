...
export function getRemainingCooldownMs(player, nowMs) {
    if (!player || player.lastActionAtMs == null)
        return 0;
    return Math.max(0, PLAYER_ACTION_COOLDOWN_MS - (nowMs - player.lastActionAtMs));
}
function isPlayerOnCooldown(players, playerId, nowMs) {
    const p = findPlayer(players, playerId);
    return getRemainingCooldownMs(p, nowMs) > 0;
}
function bumpPlayerLastAction(players, playerId, nowMs) {
    return players.map((p) => (p.id === playerId ? { ...p, lastActionAtMs: nowMs } : p));
}
function pushRecent(state, text, atMs, kind, opts) {
    const item = {
...
export function createInitialRoom(roomId, nowMs, firstPlayer) {
    ...
    switch (action.type) {
        ...
        case 'ROTATE_TILE': {
            ...
            if (tile && tile.type === 'sync_tile') {
                ...
                const withinWindow = currentTile.heldUntilMs != null && now <= currentTile.heldUntilMs;
                if (!withinWindow) {
            ...
            const game = rotated.status === 'won' && state.game.status !== 'won'
