...
export function TileView({ tile, pos, inSolution, nowMs, reactorPowered, players, onClick, onPing, recentMeta, winAnimation, }) {
    const isEmpty = tile.type === 'empty';
    const open = isEmpty ? new Set() : getOpenSides(tile);
    ...
    const overlayStrength = winAnimActive && tileIdx != null ? Math.exp(-Math.pow(tileIdx - wavePos, 2) / (2 * WAVE_SIGMA * WAVE_SIGMA)) : 0;
    ...
    const highlightActive = recentKindIsRotate &&
        playerColor != null &&
        actionAtMs != null &&
        nowMs - actionAtMs >= 0 &&
    ...
    const overlayShadow = poweredPath && overlayStrength > 0.02
        ? isReactor
    ...
    const overlayFilter = poweredPath && overlayStrength > 0.03
        ? isReactor
    ...
    const cellBackground = (() => {
        ...
        const frameVignette = 'radial-gradient(circle at 50% 50%, rgba(0,0,0,0) 58%, rgba(0,0,0,0.30) 100%)';
        if (poweredPath) {
    ...
    return (_jsx("button", { type: "button", className: classes, style: cellStyle, onClick: () => onClick(pos), onContextMenu: (e) => {
            e.preventDefault();
            if (!winAnimActive)
                onPing(pos);
        }, "aria-label": isEmpty
            ? `Empty ${shortPos(pos)}`
            : `${tile.type} ${shortPos(pos)}${clickable ? ', rotate' : ''}`, disabled: !clickable || winAnimActive, children: !isEmpty && (_jsxs("svg", { className: "tileSvg", viewBox: "0 0 100 100", role: "presentation", "aria-hidden": "true", style: {
                width: '100%',
                height: '100%',
                filter: filterValue,
                animation: heldActive
                    ...
                    : poweredPath && overlayStrength > 0.14
                        ...
                        : undefined,
            }, children: [_jsxs("g", { className: "pipe", children: [segs.map((s, i) => {
                            ...
                            return (_jsxs("g", { children: [_jsx("line", { x1: s.x1, y1: s.y1, x2: s.x2, y2: s.y2, stroke: "rgba(0,0,0,0.65)", strokeWidth: mainStroke + 4.5, strokeOpacity: edgeOpacity, strokeLinecap: "round" }), _jsx("line", { x1: s.x1, y1: s.y1, x2: s.x2, y2: s.y2, stroke: connectorColor, strokeWidth: mainStroke, strokeOpacity: mainOpacity, strokeLinecap: "round" }), _jsx("line", { x1: s.x1, y1: s.y1, x2: s.x2, y2: s.y2, stroke: effectiveConnectorHighlightColor, strokeWidth: highlightWidth, strokeOpacity: highlightOpacity, strokeLinecap: "round" }), _jsx("circle", { cx: s.x1, cy: s.y1, r: beadR, fill: connectorColor, opacity: beadOpacity }), _jsx("circle", { cx: s.x2, cy: s.y2, r: beadR, fill: connectorColor, opacity: beadOpacity })] }, i));
                        }), _jsx("circle", { cx: "50", cy: "50", r: powered || isOverloaded ? 13 + overlayStrength * 3 + nodeBoost : 10 + nodeBoost, fill: connectorColor, opacity: powered || isOverloaded ? 0.92 + overlayStrength * 0.14 : 0.78 }), _jsx("circle", { cx: "50", cy: "50", r: powered || isOverloaded ? 7 + overlayStrength * 1.2 + nodeBoost * 0.2 : 5.4 + nodeBoost * 0.2, fill: isOverloaded ? 'rgba(249,115,22,0.20)' : poweredPath ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.10)', opacity: powered || isOverloaded ? 0.55 + overlayStrength * 0.25 : 0.28 }), _jsx("circle", { cx: "50", cy: "50", r: powered || isOverloaded ? 3.8 + overlayStrength * 0.8 : 2.8, fill: isOverloaded ? 'rgba(249,115,22,0.45)' : poweredPath ? 'rgba(255,255,255,0.55)' : 'rgba(255,255,255,0.35)', opacity: powered || isOverloaded ? 0.26 + overlayStrength * 0.22 : 0.12 }), tile.type === 'straight' ||
                            tile.type === 'corner' ||
                            tile.type === 'tee' ||
                            tile.type === 'cross' ? ((() => {
                            ...
                            return (_jsxs(_Fragment, { children: [[...open].map((d) => {
                                        const end = prongEnd(d);
                                        return (_jsx("line", { x1: 50, y1: 50, x2: end.x, y2: end.y, stroke: prongStrokeColor, strokeWidth: prongStrokeWidth, strokeLinecap: "round", opacity: prongOpacity }, d));
                                    }), tile.type === 'corner' ? (() => {
                                        // Rounded inner arc to make the L-shape read as a smooth turn.
                                        if (open.has('N') && open.has('E')) {
                                            return (_jsx("path", { d: "M50 40 A12 12 0 0 1 60 50", fill: "none", stroke: prongStrokeColor, strokeWidth: prongStrokeWidth, strokeLinecap: "round", opacity: prongOpacity }));
                                        }
                                        if (open.has('E') && open.has('S')) {
                                            return (_jsx("path", { d: "M60 50 A12 12 0 0 1 50 60", fill: "none", stroke: prongStrokeColor, strokeWidth: prongStrokeWidth, strokeLinecap: "round", opacity: prongOpacity }));
                                        }
                                        if (open.has('S') && open.has('W')) {
                                            return (_jsx("path", { d: "M50 60 A12 12 0 0 1 40 50", fill: "none", stroke: prongStrokeColor, strokeWidth: prongStrokeWidth, strokeLinecap: "round", opacity: prongOpacity }));
                                        }
                                        if (open.has('W') && open.has('N')) {
                                            return (_jsx("path", { d: "M40 50 A12 12 0 0 1 50 40", fill: "none", stroke: prongStrokeColor, strokeWidth: prongStrokeWidth, strokeLinecap: "round", opacity: prongOpacity }));
                                        }
                                        return null;
                                    })() : null] }));
                        })()) : null] }), isOverloaded && !isEmpty && (_jsxs("g", { className: "overloadCracks", opacity: "0.95", children: [_jsx("path", { d: "M30 30 L70 70", stroke: "rgba(249,115,22,0.95)", strokeWidth: "7", strokeLinecap: "round" }), _jsx("path", { d: "M70 30 L30 70", stroke: "rgba(249,115,22,0.95)", strokeWidth: "7", strokeLinecap: "round", opacity: "0.85" })] })), isSyncTile && (_jsxs("g", { className: "glyph sync", children: [_jsx("circle", { cx: "50", cy: "50", r: "16", fill: heldActive ? `${holderColor}22` : 'rgba(170,59,255,0.10)', stroke: heldActive ? holderColor : 'rgba(170,59,255,0.35)', strokeWidth: heldActive ? 4 : 3 }), _jsx("circle", { cx: "50", cy: "50", r: heldActive ? 7.5 : 6, fill: heldActive ? holderColor : '#A78BFA', opacity: heldActive ? 0.95 : 0.35 }), _jsx("path", { d: "M43 52 L43 48 L47 48 L50 51 L53 48 L57 48 L57 52 L53 52 L50 55 L47 52 Z", fill: heldActive ? `${holderColor}` : 'rgba(170,59,255,0.55)', opacity: heldActive ? 0.95 : 0.55 })] })), tile.type === 'generator' && (_jsxs("g", { className: "glyph generator", children: [_jsx("circle", { cx: "50", cy: "50", r: "28", fill: winAnimActive ? 'rgba(68,215,255,0.14)' : 'rgba(68,215,255,0.08)', stroke: "#44D7FF", strokeWidth: winAnimActive ? 2.5 : 2.0, opacity: "0.95" }), _jsx("circle", { cx: "50", cy: "50", r: "24", fill: winAnimActive && (winGeneratorPulseActive || overlayStrength > 0.35)
                                ? 'rgba(68,215,255,0.72)'
                                : 'rgba(68,215,255,0.42)', stroke: "#44D7FF", strokeWidth: winAnimActive && (winGeneratorPulseActive || overlayStrength > 0.35) ? 7.2 : 6.3 }), _jsx("circle", { cx: "50", cy: "50", r: winAnimActive && (winGeneratorPulseActive || overlayStrength > 0.35) ? 14 : 11.5, fill: "#44D7FF", opacity: winAnimActive ? 0.98 : 0.92 }), _jsx("circle", { cx: "50", cy: "50", r: "17.2", fill: "none", stroke: "#44D7FF", strokeWidth: poweredPath ? 3.8 : 3.1, opacity: poweredPath ? 0.85 : 0.42 }), _jsx("circle", { cx: "50", cy: "50", r: poweredPath ? 9.6 : 8.1, fill: poweredPath ? 'rgba(125,211,252,0.72)' : 'rgba(125,211,252,0.36)', opacity: poweredPath ? 0.96 : 0.62 }), _jsx("circle", { cx: "50", cy: "50", r: poweredPath ? 5.8 : 4.7, fill: poweredPath ? 'rgba(255,255,255,0.58)' : 'rgba(255,255,255,0.30)', opacity: poweredPath ? 0.65 : 0.40 }), _jsxs("g", { opacity: winAnimActive || poweredPath ? 0.98 : 0.70, style: {
                                animation: poweredPath ? 'energyFlowPulse 1.25s ease-in-out infinite' : undefined,
                                transformOrigin: '50px 50px',
                            }, children: [_jsx("path", { d: "M50 18 C58 24 58 32 50 38 C42 32 42 24 50 18", fill: "none", stroke: "#44D7FF", strokeWidth: "4.3", strokeLinecap: "round" }), _jsx("path", { d: "M82 50 C76 42 68 42 62 50 C68 58 76 58 82 50", fill: "none", stroke: "#44D7FF", strokeWidth: "4.3", strokeLinecap: "round" }), _jsx("path", { d: "M50 82 C42 76 42 68 50 62 C58 68 58 76 50 82", fill: "none", stroke: "#44D7FF", strokeWidth: "4.3", strokeLinecap: "round" }), _jsx("path", { d: "M18 50 C24 58 32 58 38 50 C32 42 24 42 18 50", fill: "none", stroke: "#44D7FF", strokeWidth: "4.3", strokeLinecap: "round" })] })] })), tile.type === 'reactor' && (_jsxs("g", { className: "glyph reactor", style: {
                        animation: poweredPath ? 'energyFlowPulse 0.9s ease-in-out infinite' : undefined,
                        transformOrigin: '50px 50px',
                    }, children: [_jsx("circle", { cx: "50", cy: "50", r: "28", fill: winAnimActive ? 'rgba(211,155,255,0.12)' : 'rgba(211,155,255,0.07)', stroke: "#D39BFF", strokeWidth: winAnimActive ? 2.4 : 1.9, opacity: "0.95" }), _jsx("circle", { cx: "50", cy: "50", r: "24", fill: winAnimActive && (winReactorFlashActive || overlayStrength > 0.35)
                                ? 'rgba(211,155,255,0.76)'
                                : poweredPath
                                    ? 'rgba(211,155,255,0.58)'
                                    : 'rgba(211,155,255,0.18)', stroke: "#D39BFF", strokeWidth: winAnimActive && (winReactorFlashActive || overlayStrength > 0.35)
                                ? 6.2
                                : poweredPath
                                    ? 6.0
                                    : 4.9 }), _jsx("circle", { cx: "50", cy: "50", r: winAnimActive && (winReactorFlashActive || overlayStrength > 0.35)
                                ? 23
                                : poweredPath
                                    ? 19
                                    : 15, fill: "#D39BFF", opacity: "0.98" }), _jsx("circle", { cx: "50", cy: "50", r: "20.8", fill: "none", stroke: "#D39BFF", strokeWidth: poweredPath ? 4.2 : 3.1, opacity: poweredPath ? 0.86 : 0.40 }), _jsx("circle", { cx: "50", cy: "50", r: "15.8", fill: "none", stroke: poweredPath ? 'rgba(245,205,255,0.55)' : 'rgba(245,205,255,0.25)', strokeWidth: poweredPath ? 3.1 : 2.3, strokeDasharray: "4 6", opacity: poweredPath ? 0.95 : 0.55 }), _jsxs("g", { opacity: poweredPath ? 0.98 : 0.75, children: [_jsx("path", { d: "M50 30 L58 42 L50 58 L42 42 Z", fill: poweredPath ? 'rgba(211,155,255,0.70)' : 'rgba(211,155,255,0.34)' }), _jsx("path", { d: "M50 34 L55 42 L50 49 L45 42 Z", fill: poweredPath ? 'rgba(255,255,255,0.22)' : 'rgba(255,255,255,0.12)', opacity: poweredPath ? 1 : 0.85 }), _jsx("line", { x1: "50", y1: "18", x2: "50", y2: "30", stroke: "#FFFFFF", strokeOpacity: poweredPath ? 0.28 : 0.14, strokeWidth: "3.2", strokeLinecap: "round" }), _jsx("path", { d: "M33 50 Q50 40 67 50", fill: "none", stroke: poweredPath ? 'rgba(255,255,255,0.22)' : 'rgba(255,255,255,0.10)', strokeWidth: "3.2", strokeLinecap: "round" })] })] }))] })) }));
}
