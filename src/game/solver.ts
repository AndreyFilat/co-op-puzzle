import type { Board, Direction, Pos, Tile } from './types'
...
export function solveBoard(board: Board): SolveResult {
  ...
  const cleared: Board = board.map((row) => row.map(withSolvedCleared))
  if (!generatorPos) {
    return {
      board: cleared,
      poweredKeys: new Set(),
  ...
  type State = Readonly<{ pos: Pos; incomingDir: Direction | null; depth: number }>
  const stateKey = (s: State): string => `${posKey(s.pos)}|${s.incomingDir ?? 'gen'}`

  const visitedStates = new Set<string>()
  const overloadedKeys = new Set<string>()

  const queue: State[] = [{ pos: generatorPos, incomingDir: null, depth: 0 }]
  ...
  while (head < queue.length) {
    ...
    const incomingDirsThisLayerByTile = new Map<string, Set<Direction>>()
    for (const s of layer) {
      if (s.incomingDir == null) continue // ignore generator emission marker
      const k = posKey(s.pos)
      const set = incomingDirsThisLayerByTile.get(k) ?? new Set<Direction>()
      set.add(s.incomingDir)
      incomingDirsThisLayerByTile.set(k, set)
    }

    for (const [tileKey, dirs] of incomingDirsThisLayerByTile.entries()) {
      if (dirs.size > 1) overloadedKeys.add(tileKey)
    }
    ...
    for (const cur of layer) {
      ...
      const exitSides: Direction[] =
        cur.incomingDir == null ? [...curOpen] : [...curOpen].filter((d) => d !== cur.incomingDir)

      if (exitSides.length === 0) continue

      for (const exitDir of exitSides) {
        const { dr, dc } = dirDelta(exitDir)
        const nxt: Pos = { r: cur.pos.r + dr, c: cur.pos.c + dc }
        ...
        if (!nxtOpen.has(oppositeDir(exitDir))) continue

        const nxtIncomingDir = oppositeDir(exitDir)
        const nxtState: State = { pos: nxt, incomingDir: nxtIncomingDir, depth: currentDepth + 1 }
        const nxtStateKey = stateKey(nxtState)
        if (visitedStates.has(nxtStateKey)) continue
  ...
  const poweredVisited = new Set<string>()
  const q2: Pos[] = []
  ...
  while (q2.length) {
    ...
    for (const dir of curOpen) {
      ...
      if (overloadedKeys.has(key)) continue // overloaded tiles do not conduct
      if (poweredVisited.has(key)) continue
  ...
  const solvedBoard: Board = cleared.map((row, r) =>
    row.map((t, c) => {
      ...
      if (t.powered === powered && (t.overloaded === true) === overloaded) return t
