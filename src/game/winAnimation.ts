...
export type WinPathInfo = Readonly<{
  pathKeys: ReadonlyArray<string>
  generatorKey: string | null
  reactorKey: string | null
}>

function findPos(board: Board, type: string): Pos | null {
  for (let r = 0; r < board.length; r++) {
    for (let c = 0; c < board[r]!.length; c++) {
      if (board[r]![c]!.type === type) return { r, c }
    }
  }
  return null
}
 ...
 */
export function computeWinPathInfo(board: Board): WinPathInfo {
  ...
  while (q.length) {
    ...
    for (const dir of curOpen) {
      ...
      if (!inBounds(board, nxt)) continue

      const nxtTile = board[nxt.r]![nxt.c]!
      if (!nxtTile.powered) continue
  ...
  const reversed: string[] = [reactorKey]
